...
On commence par mettre à jour les packages système vers la dernière version
sudo apt update sudo apt -y upgrade

Etape 1 : Installation des packages nécessaires
On installe certains packages indispensables comme suite. 
...
Avec cette commande ci-dessus on a installé Apache2, Freeradius, PHP, et Mariadb serveur d’un seul coup.

NB: on peut aussi installer cela séparément.

Une fois Apache2 activé, on peut configurer le serveur SQL (dans ce cas, le serveur Mariadb).
...
Maintenant que nous avons configuré le serveur, nous pouvons commencer à configurer la base de données. En tapant la commande qui va suivre on demandera de saisir le mot de passe défini précédemment lors de la configuration.

sudo mysql -u root -p {mot de passe}

Maintenant que nous sommes connectés, nous devons effectuer quelques opérations. 
1-Créer la base de données
2-Créer un utilisateur pour FreeRadius
3-Donner les autorisations à l'utilisateur
Nous pouvons créer la base de données en exécutant cette commande « CREATE DATABASE » :
1-Créer la base de données
CREATE DATABASE radius;
2-Créer un utilisateur pour FreeRadius
Ensuite, nous devons créer le compte utilisateur. On doit remplacer PASSWORD par un mot de passe sécurisé.
...
Résultats de modifications attendus:
...
On change les paramètre: server=”localhost” et password=”un mot de passe simple”
